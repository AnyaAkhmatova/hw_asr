...
class RNN_with_BN(nn.Module):
    def __init__(self, rnn_type, input_size, hidden_size, num_layers, bidirectional):
        super().__init__()
...
class DeepSpeech2(BaseModel):
    def __init__(self, n_feats, n_class,
        ...
        self.rnns = Sequential(
            RNN_with_BN(self.rnn_type, self.rnn_input_size, self.hidden_size,
                        num_layers=self.num_layers, bidirectional=self.bidirectional),
            *(
                RNN_with_BN(self.rnn_type, self.hidden_size, self.hidden_size,
                            num_layers=self.num_layers, bidirectional=self.bidirectional)
